...
@MainActor
enum FoundationProgressStore {
    private static let folderName = "BC_Foundation"
    ...
    //
    // Используется там, где нужно просто прочитать сохранённый прогресс
    // (дашборд, список проектов, бюджет и т. п.).
    ...
    // MARK: - Загрузка с учётом текущего типа фундамента и шаблона
    //
    // Используем ЭТОТ метод в экране FoundationStagesScreen:
    // if let saved = FoundationProgressStore.load(projectID: project.id, typeID: typeID) { ... }
    //
    // Логика:
    // 1) читаем сохранённые стадии;
    // 2) берём актуальный шаблон из FoundationStagesProvider.loadStages(named: typeID);
    // 3) мержим: добавляем новые этапы/пункты, infoSlug обновляем, прогресс/заметки пользователя сохраняем.
    static func load(projectID: UUID, typeID: String) -> [Stage]? {
        ...
        let templateStages = FoundationStagesProvider.loadStages(named: typeID)
        guard !templateStages.isEmpty else {
            // на всякий случай: если по какой-то причине не нашли шаблон,
    ...
    private static func mergeItems(saved: [StageItem], template: [StageItem]) -> [StageItem] {
        ...
        for tmplItem in template {
            if var existing = saved.first(where: { $0.title == tmplItem.title }) {
                ...
                // чтобы новые md-файлы подтягивались.
